        ...
        if (this.isLdapMode()) {
            ...
            try {
                ...
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(e.getMessage() + ":" + RestResponseCommand.RSP_NETWORK_EXCEPTION);
            }

            if (DashboardHelper.isNull(userInfo) || DashboardHelper.isEmpty(userInfo.getUserName())) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body("Failed to authenticate the credential " + RestResponseCommand.RSP_AUTH_NO_PRIVILEGE);
            }
            ...
            try {
                sessions.addSession(userInfo.getSessionId(), whitelistUser.getWhiteUserName(),
                        whitelistUser.getWhiteUserRole(), EXPIRE_INTERNAL, userInfo.getDisplayName());
            } catch (Exception e) {

...
class RosettaRestApi:
    ...
    def _get_basic_authentication_header(self, depositAccount) -> str:
        ...
        credential = f"{depositAccount.depositUserName}-institutionCode-" f"{depositAccount.depositUserInstitute}:{depositAccount.depositUserPassword}"
    ...
    def fetch(self, depositAccount, method: str, path: str, reqBody: Any = None) -> str:
        ...
        # Prepare Headers
        headers = {"Authorization": self._get_basic_authentication_header(depositAccount), "Accept": "application/json", "Content-Type": "application/json"}
        ...
        try:
            ...
            response = requests.request(method=method, url=url, headers=headers, json=json_payload, timeout=300)  # Good practice to include a timeout

            # Error handling (Status != 200)
            if response.status_code != 200:
                err_msg = f"Failed to request: {path}, error: {response.text}. " f"Account: {depositAccount}"

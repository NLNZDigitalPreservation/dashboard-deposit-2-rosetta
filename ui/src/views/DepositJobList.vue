...
import {depositJobList, formatDatetimeFromEpochMilliSeconds, jobFilters} from '@/stores/depositjob';
...
const depositJobsView=computed(()=>{
    return depositJobList.value.map((e:any)=>({
        ...e,
        progress: calcProgressPercent(e),
        progressClassName: getProgressBarClass(e),
    }));
    // return depositJobList.value;
});
...
onBeforeMount(() => {
    jobFilters.value={
        ...
        injectionTitle: { value: null, matchMode: FilterMatchMode.CONTAINS },
        // 'appliedFlowSetting.materialFlowName': { value: null, matchMode: FilterMatchMode.CONTAINS },
...
<template>
    ...
    <!-- v-model:filters="jobFilters"
                filterDisplay="menu"
                :globalFilterFields="['injectionTitle']" -->

   <DataTable  v-model:expandedRowGroups="expandedRowGroups" :value="depositJobsView"  v-model:selection="selectedJobs" 
                dataKey="id" size="small" rowGroupMode="subheader" groupRowsBy="appliedFlowSetting.materialFlowName"
                sortMode="single" sortField="appliedFlowSetting.materialFlowName" :sortOrder="1"
                paginator :rows="50" :rowsPerPageOptions="[15, 30, 50, 100, 200]"
                scrollable scrollHeight="100%"
                
                showGridlines
                >
        <Column field="appliedFlowSetting.materialFlowName" header="MaterialFlow" frozen class="font-bold"></Column>
        ...
        <Column field="progress" header="Progress" sortable>
            <template #body="{ data }">
                <ProgressBar :value="data.progress" v-bind:class="data.progressClassName"></ProgressBar>
            </template>
        </Column>
        ...
        <template #groupheader="slotProps">
            <div class="flex align-items-center gap-2 font-bold">
                ...
                <span>{{ formatMaterialFlowGroup(slotProps.data) }}</span>
            </div>
        </template>
    </DataTable>
</template>

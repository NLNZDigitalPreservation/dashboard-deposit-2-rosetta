...
<template>
    ...
    <DataTable :value="tableData" size="small" showGridlines>
        ...
        <Column field="INGEST" header="INGEST">
            <template #body="{ data }">
                <div v-if="data.INGEST" class="bg-primary" style="width: 100%; text-align: center;">Y</div>
                <div v-else style="width: 100%; text-align: center;">-</div>
            </template>
        </Column>
        <Column field="DEPOSIT" header="DEPOSIT">
            <template #body="{ data }">
                <div v-if="data.DEPOSIT" class="bg-primary" style="width: 100%; text-align: center;">Y</div>
                <div v-else style="width: 100%; text-align: center;">-</div>
            </template>
        </Column>
        <Column field="FINALIZE" header="FINALIZE">
            <template #body="{ data }">
                <div v-if="data.FINALIZE" class="bg-primary" style="width: 100%; text-align: center;">Y</div>
                <div v-else style="width: 100%; text-align: center;">-</div>
            </template>
        </Column>
        <Column field="FINISHED" header="FINISHED">
            <template #body="{ data }">
                <div v-if="data.FINISHED" class="bg-primary" style="width: 100%; text-align: center;">Y</div>
                <div v-else style="width: 100%; text-align: center;">-</div>
            </template>
        </Column>
    </DataTable>
